// Database migration script to create tables and seed data
// Run this once to initialize the SQL Server database
...
async function migrate() {
  ...
  try {
    ...
    for (const team of defaultData.teams) {
      await pool.request()
        .input('id', sql.NVarChar(50), team.id)
        .input('name', sql.NVarChar(200), team.name)
        .input('product', sql.NVarChar(50), team.product)
        .query('INSERT INTO Teams (id, name, product) VALUES (@id, @name, @product)');
    }
    ...
    for (const sprint of defaultData.sprints) {
      await pool.request()
        .input('id', sql.NVarChar(100), sprint.id)
        .input('name', sql.NVarChar(200), sprint.name)
        .input('team', sql.NVarChar(50), sprint.team)
        .query('INSERT INTO Sprints (id, name, team) VALUES (@id, @name, @team)');
    }
    ...
    for (const metric of defaultData.metrics) {
      await pool.request()
        .input('id', sql.NVarChar(100), metric.id)
        .input('product', sql.NVarChar(50), metric.product)
        .input('team', sql.NVarChar(50), metric.team)
        .input('sprint', sql.NVarChar(100), metric.sprint)
        .input('requirementsCovered', sql.Int, metric.requirementsCovered)
        .input('testsCovered', sql.Int, metric.testsCovered)
        .input('defectsOpen', sql.Int, metric.defectsOpen)
        .input('defectsClosed', sql.Int, metric.defectsClosed)
        .input('deploymentReadiness', sql.Int, metric.deploymentReadiness)
        .input('codeQuality', sql.Int, metric.codeQuality)
        .input('timestamp', sql.DateTime, new Date(metric.timestamp))
        .query(`
          ...
          VALUES (@id, @product, @team, @sprint, @requirementsCovered, @testsCovered, 
                  @defectsOpen, @defectsClosed, @deploymentReadiness, @codeQuality, @timestamp)
        `);
    }
